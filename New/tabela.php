                  ...
                  <div class="panel">
                    ...
                    <div class="panel-body">
					  ...
					  <?php 
						...
						$sql = "SELECT `user_id`, `firstName`, `lastName`, `userName`, `email`, `pwd`, `creation_date` FROM `user` ";
						$result = $conn->query($sql);
						
						if ($result->num_rows > 0) {
						// output data of each row
						while($row = $result->fetch_assoc()) {
							print '<tr><td>.$row["user_id"].</td><td>.$row["firstName"].</td><td>.$row["lastName"].</td><td>.$row["userName"].</td><td>.$row["creation_date"].</td></tr>';
							}
						} 
						else {
							echo "0 results";
